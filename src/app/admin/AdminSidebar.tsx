...
import { FiHome, FiFileText, FiBriefcase, FiCalendar, FiChevronLeft, FiChevronRight } from "react-icons/fi";
...
export default function AdminSidebar() {
  ...
  return (
    <aside 
      className={`transition-all duration-300 ease-in-out bg-gradient-to-b from-[#f8fafc] to-[#f1f5f9] border-r border-gray-200 md:h-screen md:sticky top-0 flex flex-col shadow-sm w-full relative z-20 ${
    ...
    >
      ...
      <button 
        ...
        className="hidden md:flex absolute -right-4 top-10 w-8 h-8 bg-white border border-gray-200 shadow-sm rounded-full items-center justify-center text-gray-500 hover:text-primary hover:border-primary transition-colors z-30"
      >
      ...
      <div className={`p-6 md:p-8 flex items-center mb-4 transition-all duration-300 ${isCollapsed ? "justify-center px-4" : "gap-4"}`}>
        ...
        {!isCollapsed && (
          <div className="overflow-hidden whitespace-nowrap">
            <h1 className="font-bold text-gray-900 text-lg tracking-tight leading-tight">Admin BEM</h1>
      ...
      <nav className={`flex-1 overflow-y-auto space-y-1.5 ${isCollapsed ? "px-3" : "px-4 md:px-6"}`}>
        {!isCollapsed && (
          <p className="px-4 text-xs font-bold text-gray-400 uppercase tracking-wider mb-4 mt-2 whitespace-nowrap">Menu Utama</p>
        )}
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <Icon size={20} className={`shrink-0 ${isActive ? "text-white" : "text-gray-400 group-hover:text-primary"}`} /> 
